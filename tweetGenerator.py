...
class TweetGenerator:
    ...
    def processTweets(self):
        endingCharacters = ["!", "?", "."]
        priorWord = None # Keeps track what came before each iteration of words in tweet, when none = start new sentence
        for tweet in self.rawTweets: #iterate through all the tweets in rawTweets
            for word in tweet.split() + [None]: #iterate through all the words and mark the ending
                if priorWord is None and not word is None: #beginning of a sentence so update initialStates
                    self.initialStates.append(word)
                    priorWord = word
                elif word is None: # End of Sentence
                    self.updateDictionary(priorWord, "END_OF_SENTENCE")
                    priorWord = None
                elif word[-1] in endingCharacters: #check to see last character in word is in the special set of characters. end sentence
                    self.updateDictionary(priorWord, word)
                    self.updateDictionary(word, "END_OF_SENTENCE")
                    priorWord = None
                else: #We're in the middle of the sentence
                    self.updateDictionary(priorWord,word)
                    priorWord = word
    ...
    def saveTweetGen(self, fileName):
        allGenData = {'initialStates':self.initialStates, 'markovDictionary':self.markovDictionary, 'rawTweets':self.rawTweets}
...
class TrumpTweetGenerator(TweetGenerator):
    def __init__(self,sourceFile=None):
        ...
        self.theBestWords = ["GREAT!", "SAD!", "PATHETIC!", "RIDICULOUS!", "BAD!", "UNACCEPTABLE!", "NO!", "ENJOY!", "TERRIFIC!", "TERRIBLE!", "AMAZING!", "HORRIBLE!"]
        if sourceFile:
    ...
    def saveTweetGen(self, fileName):
        allGenData = {"initialStates":self.initialStates, 'rawTweets':self.rawTweets, 'markovDictionary':self.markovDictionary, 'theBestHashtags':self.theBestHashtags}
        self.saveJSON(allGenData, fileName)
